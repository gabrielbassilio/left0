...
var jobs = ["1", "01", "00", "10", "022", "400", "04", "Logistician", "Fundraiser", 
...
function generate_names(size=1) {
    var names = [];
    while (size > 0) {
        var job = jobs[getRandomInt(jobs.length)]
        var letter = job.charAt(0);
        var adjs = adj_by_letter[letter]
        var adj = adjs[getRandomInt(adjs.length)]
        names.push(`${job}`)
        size--
    }
    return names
